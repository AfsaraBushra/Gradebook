...
const char* inputName()
{
    ...
    while (true)
    {
        ...
        cout << "Wrong format! Please use only letters and at least one space" << endl;
    }
}
...
{
    list[nEm].setName("Afsara Bushra");		//preset data
    list[nEm].setEmail("afsarabushra@icloud.com");
    list[nEm].setID("20073362D");
    list[nEm].setMark(0, 0); list[nEm].setMark(1, 10); list[nEm].setMark(2, 20);
    nEm++;

    // intentionally faulty data
    list[nEm].setName("Wei Ting");
    list[nEm].setEmail("weiting@hotmail.com");
    list[nEm].setID("20040213D");
    list[nEm].setMark(0, 100); list[nEm].setMark(1, 90); list[nEm].setMark(2, 80);
    nEm++;

    list[nEm].setName("Chenjun Zhao");
    list[nEm].setEmail("chenjun@gmail.com");
    list[nEm].setID("20083934");
...
void GradeBookHandle::uMenu()
{
    ...
    while (true)
    {
        ...
        cout << "4. Validate Database (Part 0)" << endl;  // tests each function in the class
        cout << "5. Store Database (Distinction)" << endl;
        cout << "6. Load Database (Distinction)" << endl;
        cout << "7. Load Dummy Data" << endl;  // loads incorrect data, which can then be tested by task 0. This tests the Validate Database feature
        cout << "0. Exit" << endl;
        cin >> c;
        if (c != '0' && (c < '1' || c>'7')) cout << "Invalid choice!" << endl;
        if (c == '1') display();
        if (c == '2')
        {
            int serial;
        ...
        if (c == '5' || c == '6')
        {
            ...
            cout << "Enter Database Filename (no spaces): " << endl;
            cin >> filename;
